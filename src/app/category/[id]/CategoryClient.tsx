...
function CategoryContent({ initialId }: { initialId?: string }) {
  const params = useParams();
  const catId = (params?.id as string) || initialId || "";
...
export default function CategoryClient({ initialId }: { initialId?: string }) {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      >
        <CategoryContent initialId={initialId} />
      </Suspense>
    </div>
  );
}
